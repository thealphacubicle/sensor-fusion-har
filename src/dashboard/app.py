...
def main():
    ...
    selected_models = st.multiselect("Select model(s) to display:", available_models, default=available_models)

    df_filtered = df[df["model"].isin(selected_models)]

    # 1. Accuracy and F1 per model
    st.subheader("📊 Accuracy and F1 Score by Model Type")
    model_metrics = df_filtered.groupby("model")[["train_accuracy", "test_accuracy", "train_f1", "test_f1"]].mean().reset_index()
    fig1 = px.bar(
    ...
    if "sensor_config" in df_filtered.columns:
        ...
        sensor_group = df_filtered.groupby("sensor_config")[["test_accuracy", "test_f1"]].mean().reset_index()
        fig3 = px.bar(
